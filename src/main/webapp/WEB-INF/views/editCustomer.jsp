    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="content">
			...
			<div class="form-container">
				<h2 class="form-header">Chỉnh sửa thông tin khách hàng</h2>
				
				<div class="profile-image-container">
					<div>
						<img src="<c:url value="${khachHang.getAnh() == null ? '/resources/img_2/no_avatar.webp' : khachHang.getAnh()}"/>" alt="${khachHang.getFullname()}"/>
					</div>
				</div>
				
				<form:form class="form" action="edit-customer.htm?id=${khachHang.getMaKH()}" method="post" modelAttribute="khachHang" enctype="multipart/form-data">
					<label class="form-group" style="grid-area: first">
						Họ
						<form:input type="text" path="Ho" placeholder="Họ" value="${khachHang.getHo()}"/>
						<form:errors class="error-message" path="Ho"/>
					</label>
					<label class="form-group" style="grid-area: last">
						Tên
						<form:input type="text" path="Ten" placeholder="Tên" value="${khachHang.getTen()}"/>
						<form:errors class="error-message" path="Ten"/>
					</label>
					<label class="form-group" style="grid-area: email">
						Email
						<form:input type="text" path="Email" placeholder="abc@gmail.com" value="${khachHang.getEmail()}"/>
						<form:errors class="error-message" path="Email"/>
						<span class="error-message">${emailError}</span>
					</label>
					<label class="form-group" style="grid-area: phone">
						Số điện thoại
						<form:input type="text" path="SDT" placeholder="SĐT" value="${khachHang.getSDT()}"/>
					...
					<label class="form-group" style="grid-area: birth">
						Ngày sinh
						<form:input type="date" path="NgaySinh" placeholder="dd/mm/yyyy"  value="${khachHang.getNgaySinh()}"/>
						<form:errors class="error-message" path="NgaySinh"/>
					</label>
					<label class="form-group" style="grid-area: address">
						Địa chỉ
						<form:input type="text" path="DiaChi" placeholder="Địa chỉ" value="${khachHang.getDiaChi()}"/>
					</label>
					...
					<label class="form-group" style="grid-area: note">
						Ghi chú
						<form:input type="text" path="GhiChu" placeholder="Ghi chú"  value="${khachHang.getGhiChu()}"/>
					</label>
